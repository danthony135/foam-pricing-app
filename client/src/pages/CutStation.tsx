 ...
 *
 * Progress is saved on the server so a TV, a tablet and the office all agree.
 * Keys: → / space next · ← prev · Enter mark cut · 1-9 jump to a foam thickness.
 */
...
function SlabView({ orderId }: { orderId: number }) {
  ...
  const moBySku = useMemo(() => {
    const m: Record<string, string[]> = {};
    for (const l of (o?.lines ?? []) as any[]) (m[l.code] ??= []).push(`${l.moName ?? ''}×${l.qty}`);
    return m;
  }, [o]);
  ...
  // Jump to the first un-cut slab when the list changes.
  useEffect(() => {
  ...
  const pieceGroups = useMemo(() => {
    ...
    const acc: Record<string, { n: number; w: number; h: number }> = {};
    for (const p of cur.sheet.pieces) { const k = p.label; acc[k] = acc[k] ? { ...acc[k], n: acc[k].n + 1 } : { n: 1, w: p.w, h: p.h }; }
    return Object.entries(acc).map(([label, v]) => ({ label, ...v }));
  }, [cur]);
        ...
        {cur && (
          <>
            ...
            <aside className="flex w-80 shrink-0 flex-col gap-2 overflow-auto">
              ...
              {pieceGroups.map((g: any) => (
                <div key={g.label} className="rounded-md bg-slate-900 px-3 py-2">
                  <div className="flex items-center gap-2">
                    <span className="h-4 w-4 shrink-0 rounded-sm" style={{ background: colorFor(g.label, keys) }} />
                    <span className="flex-1 text-base"><b>{g.label.split(' ')[0]}</b> {g.label.split(' ').slice(1).join(' ')}</span>
                    <span className="text-lg font-black">{g.n}×</span>
                  </div>
                  <div className="ml-6 text-xs text-slate-400">{g.w}" × {g.h}" · {(moBySku[g.label.split(' ')[0]] ?? []).join(', ')}</div>
                </div>
              ))}
              ...
              {showList && (
                <div className="max-h-48 overflow-auto rounded-md bg-slate-900 p-2 text-xs text-slate-300">
                  {(o.lines as any[]).map((l, k) => <div key={k}>{l.moName} · <b>{l.code}</b> ×{l.qty}</div>)}
                </div>
              )}
            </aside>
          </>
