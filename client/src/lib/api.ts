...
export const api = {
  ...
  resizeSlab: (orderId: number, data: { foamId: number; slabIndex: number; lengthIn: number; widthIn: number; poly?: [number, number][] | null; x?: number; y?: number; angleDeg?: number; fill?: boolean }) => request<any>(`/foam-orders/${orderId}/slabs/resize`, { method: 'POST', body: JSON.stringify(data) }),
