  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-950 text-white">
      ...
      <footer className="flex items-center justify-between gap-3 border-t border-slate-800 px-5 py-3">
        ...
        <button className={`rounded-lg px-4 py-4 text-sm font-bold disabled:opacity-30 ${project ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`} onClick={toggleProject} disabled={!cur} title={cur ? 'P — draw this slab on the table with the projector' : 'Nothing to project — this list has no cut plan yet'}>{project ? '● Projecting' : '○ Project'}</button>
        <button className="rounded-lg bg-slate-800 px-4 py-4 text-sm font-semibold text-slate-300 hover:bg-slate-700 disabled:opacity-30" onClick={() => setMeasuring(true)} disabled={!cur || done.has(cur.key)} title={!cur ? 'Nothing to scan — this list has no cut plan yet' : done.has(cur.key) ? 'Slab already cut — untick it to scan again' : 'Scan whatever foam is on the table (full slab, partial, remnant), nest onto it and project'}>📷 Scan stock on table</button>
