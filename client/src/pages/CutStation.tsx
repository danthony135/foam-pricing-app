  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-950 text-white">
      ...
      <footer className="flex items-center justify-between gap-3 border-t border-slate-800 px-5 py-3">
        ...
        <button className="rounded-lg bg-slate-800 px-4 py-4 text-sm font-semibold text-slate-300 hover:bg-slate-700 disabled:opacity-30" onClick={() => setMeasuring(true)} disabled={!cur || done.has(cur.key)} title={!cur ? 'No slab to measure — this list has no cut plan yet' : done.has(cur.key) ? 'Slab already cut — untick it to re-measure' : "Check the slab's real size with the camera"}>📷 Measure slab</button>
